...
const genrateAccessAndRefreshToken=async(userId)=>{
        try{
            ...
            throw new ApiError(500,"Something went wrong while genrating refreh and access tocken")
        }
}
...
const loginUser=asyncHandler(async(req,res)=>{
        ...
        // find the user
        //password check
        //access and refress tocken
        // send cookie
        
    const {email,username,password}=req.body

          //if(!username || !email){  agar asa likhana hai to if(!(username || email)){ asa likhenge
     if(!username && !email){    
    ...
    //    console.log(user)
        ...
        if(!isPasswordValid){
            throw new ApiError(401,"Invailid user creadintials")
        }
...
const updateAccountDetails=asyncHandler(async(req,res)=>{
     ...
     const user=await User.findByIdAndUpdate(
        ...
        {new:true} //update hone ke bad ki information return hoti hai
    ).select("-password")
